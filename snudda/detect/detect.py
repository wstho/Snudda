...
class SnuddaDetect(object):
    ...
    def get_density_location(self, neuron, n_id,  seed):
        ...
        #     hyper_voxel_id = list(np.unique(rng.choice(self.occupied, size = n_hv)))
            ...
            #     soma_count = len(self.hv_somas[t])
            #     if soma_count <= 3:
            #         targets_list.extend(self.hv_somas[t])
            #     else:
            #         targets_list.extend(rng.choice(self.hv_somas[t], size=3, replace=False))
            
            # self.targets[n_id] = np.unique(targets_list)
            
            # dend_hv_p = dend_hv_counts/np.sum(dend_hv_counts)
            # # dend_target_hv = rng.choice(np.array(list(set(dend_field).difference(set(target_hv)))), size = , replace = False)
    ...
    def compute_bounds_old(self, hyper_voxels: dict, voxel_width: float) -> tuple:
    ...
    def compute_bounds(self, hyper_voxels: dict, voxel_width: float) -> tuple:
        """Compute the bounds and create a spatial index for hyper-voxels using vectorization."""
        # Extract origos directly as a numpy array
        origos = np.fromiter((h['origo'] for h in hyper_voxels.values()), dtype=float)
        
        # Compute bounds vectorially
        half_width = voxel_width / 2
        bounds_list = np.column_stack([
            origos - half_width, 
            origos + half_width
        ])
        
        # Create list of hyper-voxel IDs
        hid_list = list(hyper_voxels.keys())
        
        return bounds_list, hid_list
    ...
    def load_pdf(self):
        ...
        distance_prob_file = '/Users/wst/Desktop/Karolinska/Simulation/Neuron/connectivity/distance_prob.hdf5'
    ...
    def place_synapses_no_axon(self, hyper_id, voxel_space, voxel_space_ctr,
        ...
        for na_neuron_id in no_axon_neurons:
            ...
            if na_neuron["axon_density_type"] == "r":
                ...
                (na_voxel_coords, na_axon_dist) = self.no_axon_points_new_sparse(na_neuron_id)
    ...
    def get_hyper_voxel_axon_points_sparse(self,num_points=1000):
        ...
        distances, _ = tree.query(vox_idx, k=2)  
        keep_flag = np.where(distances[:, 1] < 5)[0]  
    ...
    def get_hyper_voxel_axon_points_new_sparse(self, n_id, t = 1, m = 15):
        ...
        if len(targets):
            ...
            keep_masks_by_target = [p < o for p, o in zip(p_arrays, all_o)]
    
            keep_mask = np.zeros(len(f_targs), dtype=bool)
            for target_idx, target_val in enumerate(u_targ):
                target_positions = np.where(f_targs == target_val)[0]
                keep_mask[target_positions] = keep_masks_by_target[target_idx]
    ...
    def no_axon_points_new_sparse(self, n_id): 
    ...
    def create_lookup_table(data, n_rows, data_type, num_neurons, max_synapse_type):
        ...
        lookup_table = np.zeros((data.shape[0], 3), dtype=np.int64)
            ...
            for name_type, definition in region_data["neurons"].items():
                for name, neuron_path in definition["neuron_path"].items():
                    ...
                    if "axon_density" in definition:
                        ...
                        elif axon_density_type == "new_sparse":
                            # self.prototype_neurons[name].apply("set_axon_voxel_new_sparse_density", [-1])
                            continue
    ...
    def load_neuron(self, neuron_info, use_cache=True):
        ...
        if 'axon_targets' not in neuron.__dict__:
    ...
    def distribute_neurons(self, neuron_idx=None, distribution_seeds=None, min_coord=None, max_coord=None):
        ...
        have any neurites within its border (here defined as vertices inside region)
    ...
    def distribute_neurons_axon(self, neuron_idx=None, distribution_seeds=None, min_coord=None, max_coord=None):
        ...
        try:
            ...
            for n, d_seed in zip(neurons, distribution_seeds):
                ...
                density_hyper_voxel_id = self.get_density_location(neuron=neuron, n_id = neuron_id, seed=d_seed)
